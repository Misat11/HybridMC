package misat11.hybrid.network.java.p401.util;
...
import misat11.hybrid.network.java.p401.data.chunk.BlockStorage401;
...
public class NetUtil401 implements NetUtil {
	...
	public static final NetUtil INSTANCE = new NetUtil401();

	protected NetUtil401() {
	}
	...
	public Column readColumn(byte data[], int x, int z, boolean fullChunk, boolean hasSkylight, int mask,
		...
		try {
			...
			for (int index = 0; index < chunks.length; index++) {
				if ((mask & (1 << index)) != 0) {
					BlockStorage401 blocks = new BlockStorage401(in, this);
