package misat11.hybrid.network.java.p393.util;
...
import misat11.hybrid.network.java.p393.data.chunk.BlockStorage393;
...
public class NetUtil393 implements NetUtil {
	...
	public static final NetUtil INSTANCE = new NetUtil393();

	protected NetUtil393() {
	}
	...
	public Column readColumn(byte data[], int x, int z, boolean fullChunk, boolean hasSkylight, int mask,
		...
		try {
			...
			for (int index = 0; index < chunks.length; index++) {
				if ((mask & (1 << index)) != 0) {
					BlockStorage393 blocks = new BlockStorage393(in, this);
